...
class Search extends Component {
    ...
    displayResults = (results) => {
        ...
        if(searchResults) {
            ...
            let cardJson = searchResults.reduce((total, result) => {
                let card = {}

                let location
                if (result.location) {
                    location = result.location[0]['_']
                }
        
                let updated = result['a10:updated'][0]
                let author = result['a10:author'][0]['a10:name']
            ...
            this.props.dispatch(setReduxState({ cardJson: cardJson }))

            let cardJsonArr = Object.keys(cardJson).map((key) => {
                return cardJson[key]
            })

            let cardData = cardJsonArr.map((result) => {
